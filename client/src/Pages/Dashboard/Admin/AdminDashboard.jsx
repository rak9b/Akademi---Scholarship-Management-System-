	...
	return (<>
		...
		<section className='flex justify-center'>
			<aside className=' w-[20%] border-r-2 pt-5h-[calc(100vh-70px)]   '>
				<h1 className='hidden md:block md:text-4xl text-2xl py-5 mb-5 font-bold text-center text-white max-h-max w-full bg-black border-b border-green-600/30'> Dashboard</h1>
				<ul className=' space-y-5 px-2  '>
					...
					<li>
						<NavLink className={({ isActive }) => `flex py-2 px-5 rounded-full items-center gap-2 transition-all duration-300 ${isActive ? 'bg-black text-green-400 border border-green-600/50' : 'text-gray-600 hover:bg-gray-100'}`} to="/dashboard/manage-users">
							<FaUsers></FaUsers>
					...
					<li>
						<NavLink className={({ isActive }) => `flex py-2 px-5 rounded-full items-center gap-2 transition-all duration-300 ${isActive ? 'bg-black text-green-400 border border-green-600/50' : 'text-gray-600 hover:bg-gray-100'}`} to="analytics">
							<RiBarChartBoxFill />
